"""
Enhanced document processor service for asynchronous document processing.

This is an upgraded version of the original document processor with advanced
features for processing, indexing, and monitoring documents in the RAG system.
"""
...
from ..processors.document_quality import DocumentQualityProcessor
...
class Document(Base):
    ...
    processing_info = sa.Column(sa.JSON, nullable=True)
    quality_score = sa.Column(sa.Float, nullable=True)
    language = sa.Column(sa.String, nullable=True)
    ocr_applied = sa.Column(sa.Boolean, default=False)
    ocr_quality = sa.Column(sa.Float, nullable=True)
...
class ProcessingHistory(Base):
    """Database model for document processing history."""
    
    __tablename__ = "processing_history"
    
    id = sa.Column(sa.String, primary_key=True)
    document_id = sa.Column(sa.String, sa.ForeignKey("documents.id"), nullable=False)
    stage = sa.Column(sa.String, nullable=False)
...
class ProcessingMetrics(Base):
    """Database model for processing metrics."""
    
    __tablename__ = "processing_metrics"
    
    id = sa.Column(sa.Integer, primary_key=True, autoincrement=True)
    timestamp = sa.Column(sa.DateTime, default=datetime.utcnow)
    metric_type = sa.Column(sa.String, nullable=False)
    metric_name = sa.Column(sa.String, nullable=False)
    value = sa.Column(sa.Float, nullable=False)
    unit = sa.Column(sa.String, nullable=True)
    labels = sa.Column(sa.JSON, nullable=True)


class DocumentProcessorV2:
    """
    Enhanced service for processing documents asynchronously.
    ...
    - Indexing with multiple embedding options
    - Comprehensive processing status and metrics
    ...
    def __init__(self):
        ...
        self.enable_quality_check = os.environ.get("ENABLE_QUALITY_CHECK", "true").lower() == "true"
        self.min_quality_score = float(os.environ.get("MIN_QUALITY_SCORE", "0.5"))
        self.max_processing_attempts = int(os.environ.get("MAX_PROCESSING_ATTEMPTS", "3"))
        self.parallel_processing = int(os.environ.get("PARALLEL_PROCESSING", "4"))
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Initialize index processor with configuration
            chunking_config = {
            ...
            ocr_config = {
                ...
                "temp_dir": os.path.join(self.cache_dir, "ocr")
            }
            ...
            # Initialize processing semaphore for concurrency control
            self.processing_semaphore = asyncio.Semaphore(self.parallel_processing)
            
            self.initialized = True
            
            # Record initialization in metrics
            await self._record_metric("system", "initialization", 1.0)
    ...
    async def check_minio_for_new_files(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            await self._record_metric("error", "minio_check", 1.0, 
                                     labels={"error": str(e)})
            return []
    ...
    async def get_retry_documents(self, limit: int) -> List[Document]:
        """
        Get documents that need to be retried.
        
        Args:
            limit: Maximum number of documents to return
            
        Returns:
            List of documents to retry
        """
        try:
            async with self.db_session() as session:
                # Get documents in error state with fewer than max attempts
                query = sa.select(Document).where(
                    (Document.status == "error") &
                    (sa.func.coalesce(sa.cast(
                        sa.func.json_extract(Document.processing_info, '$.attempts'), 
                        sa.Integer), 0) < self.max_processing_attempts)
                ).order_by(Document.updated_at).limit(limit)
                
                result = await session.execute(query)
                return list(result.scalars().all())
                
        except Exception as e:
            logger.error(f"Error getting retry documents: {e}")
            return []
            ...
            async with self.db_session() as session:
                for file_path in file_paths:
                    try:
                        ...
                        # Record metric
                        await self._record_metric("document", "registered", 1.0, 
                                                 labels={"type": file_type})
            ...
            async with self.db_session() as session:
                for file_info in file_infos:
                    try:
                        ...
                        # Record metric
                        await self._record_metric("document", "registered", 1.0, 
                                                 labels={"type": file_type, "source": "minio"})
    ...
    async def process_document(self, doc: Document) -> bool:
        ...
        async with self.processing_semaphore:
            ...
            processing_id = str(hashlib.md5(f"{doc.id}_{process_start_time}".encode()).hexdigest())
            ...
            try:
                ...
                await self._add_processing_history(
                    processing_id,
                    doc.id,
                    "document_processing",
                    "started",
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Process file with FileConnector and create Document object for processor
                    ...
                    # Record connector processing time
                    await self._record_metric("processing_time", "connector", 
                                             time.time() - connector_start)
                    
                    # Process document with index processor
                    processing_start = time.time()
                    success, processing_info = await self.index_processor.process_document(document)
                    
                    # Record processing time
                    await self._record_metric("processing_time", "indexing", 
                                             time.time() - processing_start)
                    
                    if not success:
                        raise ValueError(f"Failed to index document: {doc.filename}")
                    ...
                    async with self.db_session() as session:
                        ...
                        if db_doc:
                            ...
                            db_doc.chunk_count = processing_info.get("chunk_count", 0)
                            db_doc.processing_info = processing_info
                            
                            # Extract and store key information
                            if "quality_info" in processing_info:
                                db_doc.quality_score = processing_info["quality_info"].get("quality_score", 0.0)
                                db_doc.language = processing_info["quality_info"].get("language", "unknown")
                            
                            if "ocr_info" in processing_info:
                                db_doc.ocr_applied = True
                                db_doc.ocr_quality = processing_info["ocr_info"].get("confidence", 0.0)
                    ...
                    # Record processing completion in history
                    ...
                    await self._add_processing_history(
                        ...
                        start_time=process_start_time,
                        end_time=time.time(),
                        details={"chunks": processing_info.get("chunk_count", 0)}
                    )
                    
                    # Record metrics
                    await self._record_metric("document", "processed", 1.0, 
                                             labels={"type": doc.file_type})
                    await self._record_metric("processing_time", "total", process_duration)
                    await self._record_metric("chunks", "generated", 
                                             processing_info.get("chunk_count", 0))
                    
                    logger.info(f"Document {doc.id} processed successfully with "
                               f"{processing_info.get('chunk_count', 0)} chunks")
                    return True
                ...
                # Record processing failure in history
                await self._add_processing_history(
                    processing_id,
                    doc.id,
                    "document_processing",
                ...
                async with self.db_session() as session:
                    ...
                    if db_doc and db_doc.processing_info and "attempts" in db_doc.processing_info:
                        attempt_count = db_doc.processing_info["attempts"] + 1
                ...
                try:
                    async with self.db_session() as session:
                        ...
                        if db_doc:
                            ...
                            # Update processing info with attempt count
                            processing_info = db_doc.processing_info or {}
                            processing_info["attempts"] = attempt_count
                            processing_info["last_error"] = str(e)
                            processing_info["last_error_time"] = datetime.utcnow().isoformat()
                            db_doc.processing_info = processing_info
                ...
                # Record error metric
                await self._record_metric("error", "document_processing", 1.0, 
                                         labels={"type": doc.file_type})
    ...
    async def _add_processing_history(self, 
                                    history_id: str,
                                    document_id: str, 
                                    stage: str, 
                                    status: str,
        ...
        Args:
            history_id: Unique ID for this history record
        ...
        try:
            async with self.db_session() as session:
                ...
                history = ProcessingHistory(
                    ...
                    stage=stage,
                    status=status,
                    started_at=datetime.fromtimestamp(start_time) if start_time else datetime.utcnow(),
                    completed_at=datetime.fromtimestamp(end_time) if end_time else None,
                    duration=duration,
                    details=details,
                    error=error
                )
    ...
    async def _record_metric(self, 
        ...
        Args:
            metric_type: Type of metric (processing_time, error, document, etc.)
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if pending_docs:
                        ...
                        processing_tasks = [
                            self.process_document(doc) for doc in pending_docs
                        ]
                        
                        await asyncio.gather(*processing_tasks)
                    
                    # Check for documents that need to be retried
                    retry_docs = await self.get_retry_documents(self.batch_size)
                    
                    if retry_docs:
                        logger.info(f"Retrying {len(retry_docs)} failed documents")
                        
                        # Process retry documents
                        retry_tasks = [
                            self.process_document(doc) for doc in retry_docs
                        ]
                        
                        await asyncio.gather(*retry_tasks)
                    
                    # Record system health metric
                    await self._record_metric("system", "health", 1.0)
                    
                except Exception as e:
                    logger.error(f"Error in document processor loop: {e}")
                    await self._record_metric("error", "processor_loop", 1.0, 
                                             labels={"error": str(e)})
            ...
            await self._record_metric("error", "fatal", 1.0, 
                                     labels={"error": str(e)})
        finally:
            # Close connections
            if self.indexer:
